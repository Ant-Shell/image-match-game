...
const flipCard = (event:Event) => {
  ...
  if ((event.target as HTMLImageElement).alt === props.shuffledPhoto.position.toString()) {
...
<template>
  <button class="border-black border-2 h-full w-full rounded-lg" :disabled="props.shuffledPhoto?.isLocked" @click="flipCard">
    ...
    :src="shuffledPhoto?.src.medium" :alt="props.shuffledPhoto?.position">
    ...
    <img v-else class="h-full w-full rounded-lg hover:mt-[-5px] hover:shadow-md
     hover:border-sky-700 hover:shadow-cyan-500/50 cursor-pointer" :src="cardBlue" :alt="props.shuffledPhoto?.position">
  </button>
</template>
